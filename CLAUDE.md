...
| **Error & Empty States** *(hidden)* | Feedback-message gallery, **not in the rail** — reachable only at `/states`. Reproduces every `st.error`/`st.warning`/`st.info`/`st.success` the app shows (exact text + icon, each labelled with its call site) so they can be screenshotted for the report without recreating each condition — incl. the full-page **DB-unreachable** block. Strings mirror the live call sites and are kept in step by hand. | — |
...
reachable only at `/theme`, as is the hidden **Error & Empty States**
page (`visibility="hidden"`, icon `report`) at `/states`.
...
of exception 1 for why it isn't in `dashboard.py`), for chrome behaviours
Streamlit gives no config hook for, all verified theme-safe in light
**and** dark:
